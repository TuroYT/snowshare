 ...
 */
export function isValidExpirationDate(expiresAt: Date | string): {
  valid: boolean;
  error?: string;
} {
 ...
 */
export function isValidAnonExpirationDate(expiresAt: Date | string): {
  valid: boolean;
  error?: string;
} {
  ...
  if (date > maxExpiry) {
    return {
      valid: false,
      error: `Anonymous users cannot create shares expiring beyond ${MAX_ANON_EXPIRY_DAYS} days`,
    };
  }
