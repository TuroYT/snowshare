...
- **API keys**: keys are hashed with SHA-256 again. Keys created with a recent release could not authenticate; recreate them from your profile if needed. A client that sends too many invalid keys now receives `429` (it used to be treated as anonymous).
