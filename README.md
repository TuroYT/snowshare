...
> **Reverse proxy:** the client IP (quotas, rate limiting, access logs) is taken from the last `X-Forwarded-For` entry, i.e. the one appended by your proxy. If several proxies chain in front of SnowShare (e.g. Cloudflare → nginx), set `TRUSTED_PROXY_COUNT` to their number.
